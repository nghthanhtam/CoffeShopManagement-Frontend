...
export const getStorageReports = (show = 5, page = 1, query) => (dispatch, getState) => {
  let newQuery = ''
...
export const getSearchStorageReports = query => dispatch => {
  ...
  axios
    .get(`${process.env.REACT_APP_BACKEND_HOST}/api/storagereport/search/${newQuery}`,
    )
...
export const deleteStorageReport = id => (dispatch, getState) => {
  axios
    .delete(`${process.env.REACT_APP_BACKEND_HOST}/api/storagereport/${id}`,
      tokenConfig(getState))
...
export const addStorageReport = newItem => (dispatch, getState) => {
  ...
  axios
    .post(`${process.env.REACT_APP_BACKEND_HOST}/api/storagereport/`, newItem,
      tokenConfig(getState))
...
export const setStorageReportsLoading = () => {
  return {
