...
export const getInvoices = (show = 5, page = 1, query) => (
  ...
  axios
    .get(
      `${process.env.REACT_APP_BACKEND_HOST}/api/invoice/${show}/${page}/${newQuery}`, tokenConfig(getState)
    )
...
export const getAllInvoices = (query) => (dispatch) => {
  ...
  axios
    ...
    .then(response => dispatch({ type: GET_ALL_INVOICES, payload: response.data }))
    .catch(er => console.log(er.response));
};

export const deleteInvoice = id => (dispatch, getState) => {
  axios
    .delete(`${process.env.REACT_APP_BACKEND_HOST}/api/invoice/${id}`, tokenConfig(getState))
    .then(response => {
      dispatch({
        type: DELETE_INVOICE,
...
export const addInvoice = newInvoice => (dispatch, getState) => {
  axios
    .post(`${process.env.REACT_APP_BACKEND_HOST}/api/invoice/`, newInvoice, tokenConfig(getState))
    .then(response => {
      dispatch({
        type: ADD_INVOICE,
        payload: newInvoice,
