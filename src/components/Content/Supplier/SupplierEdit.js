...
class SupplierEdit extends Component {
  ...
  validateName(name) {
    ...
    return new RegExp(
      /^[a-zA-ZàáạâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýỳżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ]+$/
    ).test(name); // name chi bao gom chu
  }

  validateAddress(address) {
    ///return new RegExp(/[,#-\/\s\!\@\$.....]/gi);
    return new RegExp(
      /^[A-Za-z0-9àáạâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýỳżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ]+$/
    ).test(address); // address chi bao gom chu va so
  }
...
export default connect(mapStateToProps, { showNoti, updateSupplier })(
  SupplierEdit
);
